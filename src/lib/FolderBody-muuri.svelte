<script lang="ts">
    ...
    import Grid from './Grid-muuri.svelte';
    ...
    function onDropFolder(itemsData: any[]) {
        folderIdsStore.set(itemsData.map((data: any) => data.id));
	}

    function onDropPlaylist(itemsData: any[]) {
        playlistIdsStore.set(itemsData.map((data: any) => data.id));
	}
</script>
